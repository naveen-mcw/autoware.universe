  ...
  const char * name, const void * data, nvinfer1::DataType dataType, int count, cudaStream_t stream)
{
  ...
  switch (dataType) {
    ...
    case nvinfer1::DataType::kHALF: {
      ...
      cudaStreamSynchronize(stream);
      for (int i = 0; i < count; ++i) host_values[i] = __half2float(host_half[i]);
      delete[] host_half;
      break;
    }
    case nvinfer1::DataType::kINT8: {
      ...
      cudaStreamSynchronize(stream);
      for (int i = 0; i < count; ++i) host_values[i] = static_cast<float>(host_int8[i]);
      delete[] host_int8;
      break;
    }
    case nvinfer1::DataType::kINT32: {
      ...
      cudaStreamSynchronize(stream);
      for (int i = 0; i < count; ++i) host_values[i] = static_cast<float>(host_int32[i]);
      delete[] host_int32;
      break;
    }
    default:
