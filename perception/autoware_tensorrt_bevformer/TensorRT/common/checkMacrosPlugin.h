 ...
 * reserved. SPDX-FileCopyrightText: Portions Copyright (c) 2022 Derry Lin
 * SPDX-FileCopyrightText: Portions Copyright (c) 2025 The Autoware Contributors
 *
...
public:
      TRTException(
        const char * fl, const char * fn, int ln, int st, const char * msg,
        const char * nm)
...
#define PLUGIN_API_CHECK_ENUM_RANGE(Type, val) \
  PLUGIN_API_CHECK( \
    ...
    static_cast < int > (val) < EnumMax < Type > ())
